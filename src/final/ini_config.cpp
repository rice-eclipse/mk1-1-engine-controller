...
po::variables_map init_config(unsigned int *port, bool *use_gitvc, int *time_between_gitvc, int *gitvc_wait_time, std::vector<int> *gitvc_times, bool *pressure_shutoff, float *pressure_slope, float *pressure_yint, int *pressure_max, int *pressure_min, unsigned int *preignite_ms, unsigned int *hotflow_ms)
{
    ...
    desc.add_options()
        ...
        ("Control.Time_between_gitvc", po::value<int>(time_between_gitvc)->required(), "set time between gitvc valve actuations")
        ("Control.Gitvc_wait_time", po::value<int>(gitvc_wait_time)->required(), "set length of ignition before gitvc")
